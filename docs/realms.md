...
1. **Preflight** — at least one OTHER R1 peer must respond to `/v1/health` (so agents have failover) AND the destination URL must respond to `/v1/health`. Skipped under `--force` (with explicit warning).
