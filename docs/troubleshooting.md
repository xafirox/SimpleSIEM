...
- **Bulk export / shipping**: `simplesiem query --since 7d` streams raw JSONL to stdout. Pipe into `jq`, gzip + copy, or any log shipper directly. The on-disk files are also standard JSONL and safe to tail/rotate externally — but if you mutate them, the hash chain will break and `verify` will flag every line after the change.
...
- **Tombstones are additive across realm peers.** Revoking on one peer reaches every peer within `realm.sync_interval_seconds` (default 60s). Removing a tombstone does NOT propagate; an accidental revocation must be undone on every peer's `config.json` manually.
