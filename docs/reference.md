...
| `server.realm.pending_join_peer` / `pending_join_psk` | single-use directive queued by master-driven server migration. The daemon's pending-join watcher picks them up on its next config-watch tick and runs the realm-join handshake automatically. Cleared once the join completes. |
| `server.realm.pending_master_collector_psk` | single-use directive set by `/v1/master/collector-directive` when an enrolling master wants the realm's existing collector to adopt it. Surfaced once via `/v1/sync/config` to the paired collector (only when the calling cert CN matches `server.collector_cn`), then cleared. The collector writes the PSK to `<state>/master_promote.psk` and the auto-promote loop re-pairs it with the master URL. |
| `server.realm.pending_collector_demote` | single-use boolean set by `/v1/master/collector-directive` when an enrolling master ALREADY has its own collector and the realm collector must yield. Surfaced once via `/v1/sync/config` (same caller-CN gate); the collector stages `<state>/pending_demote_to_server` and emits a critical `meta:collector_master_demote_directive` event with the exact follow-up command. |
...
| `POST /v1/master/collector-directive` | master → server | One-shot directive about the server's paired collector. Body: `{"master_collector_psk"?: "...", "demote_to_server"?: true}`. Exactly one of the two should be set. mTLS-gated against `server.master_cns`; refuses with `409 Conflict` when the server has no paired collector. The directive is staged in `realm.pending_master_collector_psk` / `realm.pending_collector_demote` and surfaced ONCE via `/v1/sync/config` to the paired collector (only when the calling cert CN matches `server.collector_cn`), then cleared. Driven by the master-enroll collector-arbitration flow described in [master.md](master.md#collector-arbitration-on-enroll). |
