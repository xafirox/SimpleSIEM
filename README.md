...
- **Ingesting logs from firewalls / switches / IoT devices?** → [docs/network-ingest.md](docs/network-ingest.md)
...
- **Single file** per target (~4 MB). No Python, no shell wrappers, no runtime deps.
...
- **Identity guard** — the server tracks per-cert `(IP, ts)` and rejects a second daemon presenting the same client cert from a different IP within 60 s with HTTP 409 + `meta:identity_conflict`, catching the canonical "I restored a backup while the original was still running" mistake. Bearer-token-only mode applies the same dedup keyed on `bearer:<agent_id>` so leaked tokens can't authenticate two daemons concurrently.
...
- **Network device ingest with sticky-IP allowlist + attack-pattern detection** — server / master modes ingest syslog from non-SimpleSIEM devices (firewalls, switches, IoT) over UDP / TCP / RFC 5425 TLS. **Enabled by default** with an RFC 5425 TLS listener bound on `:6514` (server) / `:6515` (master) and an auto-generated self-signed cert. Frames that fail allowlist validation are NOT dropped — they're persisted in a separate `<log_dir>/_unauthenticated/syslog/` quarantine with `authenticated: false` so investigators see attack attempts; `simplesiem tail` renders these with a `[UNAUTH]` marker. The rule engine fires only on authenticated frames, so a rogue source can't fabricate fake alerts by spamming rule-matching content. Every frame goes through a built-in **attack-pattern detector** (SQL injection, command injection, Log4Shell `${jndi:`, format-string `%n`, path traversal, XSS, XXE / DOCTYPE, LDAP injection, ANSI-escape, null-byte, overlong UTF-8, buffer-flood probes, HTTP-in-syslog protocol smuggling) — hits emit a `severity: high` alert tagged with the matching MITRE ATT&CK tactic + technique ID, regardless of whether the source is allowlisted. Operators extend the pattern set via a hot-reloadable `<config>/attack-patterns.json` sidecar. Every authorised device is identified by a captured `(IP, MAC)` tuple, ARP-resolved at trust-time. Default-gateway is auto-discovered + allowlisted on install/convert; agents AND collectors report their own gateway up to their source (server or master) so changes propagate without operator action. `--vendor` is required on every add — the bundled catalog covers pfSense, FortiGate, Cisco IOS, Cisco Meraki, SonicWall, Ubiquiti, HPE Aruba (all of which require TLS), plus a generic `other` catch-all for cleartext-only legacy gear. Three TLS cert modes: reuse the SimpleSIEM server cert, operator-provided cert, or auto-generated self-signed (fingerprint printed for vendor pinning). Bidirectional master ↔ server sync with per-server consent (`master_can_push_allowlist`); edits from either tier propagate to the realm with a pending-queue dispatcher for outage recovery. Atomic JSON edits with last-good-state revert on malformed reload, hot-reloadable in <2s. Listener refused on agent / standalone / collector modes. → [docs/network-ingest.md](docs/network-ingest.md)